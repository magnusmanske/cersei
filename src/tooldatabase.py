...
class ToolDatabase:
    LETTER2TYPE = {"Q": "item", "P": "property", "L": "lexeme"}
    ...
    def get_entry_id_for_scraper_and_id(self, scraper_id, source_id) -> int:
        ...
        return self.__get_or_create(
            "entry", ("scraper_id", "source_text_id"), (scraper_id, source_text_id)
        )
    ...
    def get_current_revision_id(self, entry_id) -> int:
        with self.get_cursor() as cursor:
            ...
            if result is None:
                raise Exception(
                    "Expected entry ID '" + str(entry_id) + "' is not in database"
                )
            return result[0]
    ...
    def get_old_revision_id_query(self, scraper_id: int):
        sql = (
            "SELECT revision.id FROM revision,entry WHERE entry.scraper_id="
            + str(scraper_id)
            + " AND revision.entry_id=entry.id AND revision.id!=entry.current_revision_id"
        )
        return sql
    ...
    def clear_old_revisions_in_table(self, scraper_id: int, table: str):
        sql = (
            ...
            + self.get_old_revision_id_query(scraper_id)
            + ")"
        )
        with self.get_cursor() as cursor:
            cursor.execute(sql, ())
    ...
    def source_ids_in_wikidata_but_not_here(
        self, scraper_id: int, source2wiki: dict
    ) -> list:
        with self.get_dict_cursor() as cursor:
            all_source_ids = {}
            ...
            sql = (
                "SELECT `source_id` FROM `vw_entry` WHERE `scraper_id`=%s AND source_id IN ("
                + ",".join(placeholders)
                + ")"
            )
    ...
    def get_wikidata_mappings_for_source_ids(self, scraper_id, source_ids):
        with self.get_dict_cursor() as cursor:
            ...
            ret = {}
            for row in rows:
                row["source_id"] = row["source_id"].decode(
    ...
    def construct_item(self, item_type, item_id):
        ...
        raise Exception(
            "ToolDatabase::construct_item: Unknown item type '" + item_type + "'"
        )
    ...
    def split_item(self, item):
        ...
        if letter not in self.LETTER2TYPE:
            raise Exception(
                "ToolDatabase::split_item: '" + item + "' has unknown letter"
            )
        item_id = item[1:]
        if not item_id.isnumeric():
            raise Exception(
                "ToolDatabase::split_item: '" + item + "' has non-numeric part"
            )
    ...
    def get_entry_ids_for_source_ids(self, scraper_id, source_ids):
        with self.get_dict_cursor() as cursor:
            ...
            ret = {}
            for row in rows:
                ret[row["source_id"].decode(encoding="utf8", errors="ignore")] = int(
                    row["entry_id"]
                )
            return ret
    ...
    def get_entities(self, entry_ids):
        ...
        sql = (
            "SELECT `entry`.`id`,`json`,`current_revision_id` FROM `entry`,`revision_item` WHERE `entry`.`current_revision_id`=`revision_item`.`revision_id` AND `entry`.`id` IN ("
            + ",".join(entry_ids)
            + ")"
        )
        with self.get_cursor() as cursor:
    ...
    def query_entries(self, j):
        conditions = [
            "`t0`.`current_revision_id`=`t1`.`revision_id`",
            "`t0`.`current_revision_id`=`t2`.`id`",
        ]
        ...
        if "links" in j:
            for prop, target in j["links"]:
                ...
                conditions.append(
                    f" `t0`.`current_revision_id`=`{table}`.`revision_id` AND `{table}`.`property`={prop} AND `{table}`.`q`=%s"
                )
