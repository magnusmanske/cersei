    ...
    def generate_qs(self):
        ...
        with db.connection.cursor() as cursor:
            ...
            for row in cursor:
                ...
                if (
                    len(batch) >= 25
                ):  # (less than 50 because potentially more than one candidate item per row)
        ...
        for row in batch:
            for candidate_qs in row[fields["candidate_qs"]].split(","):
                ...
                if institution_item != str(row[fields["wd_prop_item"]]):
                    self.log("Skipping, wrong institution")
                    continue
                ...
                if self.check_work_label:
                    ...
                    for language in entity["labels"]:
                        if (
                            work_label
                            == entity["labels"][language]["value"].strip().lower()
                        ):
                            has_correct_label = True
                    if not has_correct_label:
                        self.log(
                            f"https://www.wikidata.org/wiki/{q} has the correct institution and item ID, but no matching label ({work_label})"
                        )
                        continue
                ...
                if self.check_creator:
                    ...
                    if row[fields["creator_wd_item"]] is None:
                        if self.require_matched_creator:
                            self.log("Matched creator required but not found")
                            continue
                        else:
                ...
                qs_command = [
                    ...
                    f'"{source_id}"',
                    "S459",
                    "Q123778298",  # determination method: CERSEI
                    "S248",
                    f"Q{row[fields['wd_prop_item']]}",
                    "S813",
                    today,
                ]
                yield qs_command
        ...
        if "claims" in entity and prop in entity["claims"]:
            for claim in entity["claims"][prop]:
                if (
                    ...
                    and "value" in claim["mainsnak"]["datavalue"]
                    and "id" in claim["mainsnak"]["datavalue"]["value"]
                ):
