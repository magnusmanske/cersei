...
class WikidataClaim(metaclass=abc.ABCMeta):
    ...
    def get_base_dict(self):
        d = {
            "mainsnak": {"snaktype": "value", "property": self.property},
            "type": "statement",
            "rank": self.rank,
        }
...
class WikidataClaimEntity(WikidataClaim):
    ...
    def as_dict(self):
        d = self.get_base_dict()
        d["mainsnak"]["datavalue"] = {
            "value": {"entity-type": self.entity_type.name.lower(), "id": self.entity},
...
class WikidataClaimQuantity(WikidataClaim):
    ...
    def as_dict(self):
        d = self.get_base_dict()
        d["mainsnak"]["datavalue"] = {
            "value": {"amount": f"{self.amount}"},
            "type": "quantity",
        }
        if self.unit is not None:
            d["mainsnak"]["datavalue"]["value"][
                "unit"
            ] = f"http://www.wikidata.org/entity/{self.unit}"
        return d
...
class WikidataEntity:
    ...
    def as_dict(self):
        ...
        for alias in self.aliases:
            ...
            j["aliases"][alias.language].append(
                {"language": alias.language, "value": alias.value}
            )
        for description in self.descriptions:
            j["descriptions"][description.language] = {
                "language": description.language,
                "value": description.value,
            }
        for sitelink in self.sitelinks:
            j["sitelinks"][sitelink.wiki] = {
                "wiki": sitelink.wiki,
